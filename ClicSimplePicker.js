(function( $ ){
	...
	function drawCommandRow(app) {
		var commandRow = $('<div class="commandRow"></div>');
			...
			commandRow.appendTo(app.ui.simplePicker);
	}
	...
	function getApp(control,options) {
		var app = $(control).data('ClicSimplePicker');
    	if (!app) {
    		var settings = getSettings(options)
    		$(control).data('ClicSimplePicker', {
    			settings:settings,
       			state: {
			...
			app = $(control).data('ClicSimplePicker');
    	}
    	return app;
	}
  ...
  $.fn.ClicSimplePicker = function(method) {
	...
	if ( methods[method] ) {
		...
		$.error( 'Method ' +  method + ' does not exist on ClicSimplePicker' );
	}
  };
})( jQuery );